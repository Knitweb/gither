# Agent Identity Issuance for Gither and vBank

## Abstract

Gither and vBank need two different protections that should not be collapsed into one
mechanism.
...
- vBank needs strong accountability for agents and people, but it must not publish raw
...
3. let vBank issue traceable agent identity attestations only after a regulated or
...
`vbank` currently consumes personhood tickets and scope nullifiers, and its ballot record
...
## Proposed vBank architecture

vBank should become an issuer of **agent identity attestations**, but not a warehouse of raw
identity artifacts.
...
Gither/vBank problem, that makes ML-DSA the correct primary target for:
...
- provider-to-vBank secure handoff;
...
- vBank issuer attestations require ML-DSA;
...
3. vBank or Gither releases provider receipts when issuer work is validated.
...
1. Keep vBank public records PII-free.
...
6. Add vBank support for traceable agent identity attestations backed by provider-issued
   evidence envelopes.
...
- vBank becomes the issuer of traceable agent identity attestations;
...
- vBank current architecture: local repo `vbank/docs/ARCHITECTURE.md`
